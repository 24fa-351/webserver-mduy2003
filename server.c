...
pthread_mutex_t stats_mutex
    = PTHREAD_MUTEX_INITIALIZER; // Ensures access to global variables is
                                 // thread-safe
...
void handle_static(int client_fd, const char* file_path)
{
    ...
    send_response(client_fd, "HTTP/1.1 200 OK", "application/octet-stream",
        file_data, file_size);
    free(file_data);
}
...
void handle_stats(int client_fd)
{
    ...
    send_response(
        client_fd, "HTTP/1.1 200 OK", "text/html", response_body, body_len);
}
...
void* handle_connection(void* client)
{
    ...
    if (strcmp(method, "GET") != 0) {
        ...
        send_response(client_fd, "HTTP/1.1 405 Method Not Allowed", "text/html",
            method_not_allowed, strlen(method_not_allowed));
        close(client_fd);
        return NULL;
    }
    ...
    if (strncmp(path, "/static", STATIC_LEN) == 0) {
        ...
        if (query) {
            ...
            send_response(client_fd, "HTTP/1.1 400 Bad Request", "text/html",
                bad_request, strlen(bad_request));
        }
    } else {
        const char* not_found
            = "<html><body><h1>404 Not Found</h1></body></html>";
